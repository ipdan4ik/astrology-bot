...
async def build_buy_menu(
    session, *, tenant_id: int, i18n: Translator
    ...
    for s in subs:
        builder.button(
            text=await i18n("buy.plan_subscription", name=s.name, price=s.price_cents),
...
async def show_buy_menu(message: Message, tenant_id: int, i18n: Translator) -> None:
    async with get_sessionmaker()() as session:
        text, kb = await build_buy_menu(session, tenant_id=tenant_id, i18n=i18n)
    if not kb.inline_keyboard:
        await message.answer(await i18n("buy.no_plans"))
        return
    await message.answer(text, reply_markup=kb)


@router.message(Command("buy"))
async def on_buy_command(message: Message, account: Account, i18n: Translator) -> None:
    await show_buy_menu(message, account.tenant_id, i18n)


@router.callback_query(BuyCb.filter(F.action == "open"))
async def on_buy_open(query: CallbackQuery, account: Account, i18n: Translator) -> None:
    await show_buy_menu(query.message, account.tenant_id, i18n)
    await query.answer()


async def _invoice_description(i18n: Translator, kind: str, plan) -> str:
    if kind == "subscription":
        return await i18n("buy.invoice_subscription", period_days=plan.period_days)
    return await i18n("buy.invoice_package", count=plan.request_count)


@router.callback_query(BuyCb.filter(F.action == "pick"))
async def on_buy_pick(
    query: CallbackQuery, callback_data: BuyCb, bot: Bot, account: Account, i18n: Translator
) -> None:
    ...
    async with get_sessionmaker()() as session:
        ...
        if plan is None:
            await query.answer(await i18n("buy.plan_unavailable"), show_alert=True)
            return
    ...
    await bot.send_invoice(
        chat_id=chat_id,
        title=plan.name,
        description=await _invoice_description(i18n, callback_data.kind, plan),
...
async def on_successful_payment(message: Message, i18n: Translator) -> None:
    ...
    if credited:
        await message.answer(await i18n("buy.payment_success"))
    else:
        await message.answer(await i18n("buy.payment_already_credited"))
